...
3b. Read every section of `/mnt/c/MCP/Inbox/Master Backlog.md` whose heading contains "body of work". For each one, report: checkboxes flipped since the last digest, the next unchecked item verbatim, and whether its own Observability stall signal has fired (check it against the territory, e.g. `git -C <repo> log -1 --format=%ad --date=short`, not from the prior digest). Give each body of work its own line under In Progress (or Blocked, if its stall signal fired). (Added 2026-09-24: the music experiment was designed 08-08 but no digest surfaced it for seven weeks, because this job never read the backlog.)
4. Verify inherited rows against the territory before repeating them. For every Blocked or In Progress item carried from the prior digest that names a file, artifact, branch, or path: check the filesystem first (ls, test, git log) before restating its status. If the named artifact exists on disk, do not carry the row forward — move it to Completed, note the date found, and flag that the tracker was stale. Never increment a row age without re-verifying this week that the blocker still holds. A blocked row is a claim about the territory, not a fact inherited from the prior digest. (Rule added 2026-08-18: DESIGN-HISTORY.md sat finished on disk for 18 days while digests escalated it as not started.) Paths quoted in vault notes without a leading slash are vault-relative: resolve them under /mnt/c/MCP, never under the home directory. (Added 2026-09-19: the 09-06 digest searched /home/rbr01 for `job search/2026 ai pm/target_list.md`, found nothing, and reported the file missing while it sat in the vault.)
